/*--------------------------------------------------------------------------
 * Copyright (C) 1992-2008 Dallas Semiconductor Corporation. 
 ...
 *--------------------------------------------------------------------------
 *
 *  TEMPVC64.C - This utility uses TMEX65 to view a read the temperature from 
 *               a DS28EA00. It requires the TMEX65 drivers to be present.
 *
...
void main(short argc, char **argv)
{
   ...
   {
       printf("\nusage: tempvc64 <PortNum> \n"
              "  - read the temperature from a DS28EA00 1-Wire Digital Thermometer\n"
   ...
   for (;;)
   {
      ...
      if (session_handle > 0)  
      {
         ...
         {
            printf("Searching for a DS28EA00...\n\n");
            
            /* look for the first temperature iButton (family type 0x42) */
            if (FindFirstFamily(0x42,session_handle))
            {             
            ...
            {
               printf("DS28EA00 not found on 1-Wire network!"); 
               break;
            }
         }   
...
short ReadTemperature(long session_handle, float far *current_temp)
{
   ...
   tsht = rbuf[0] | (rbuf[1] << 8);

   *current_temp = (float)tsht/(float)16.0;
...
/*------------------------------------------------------------------------
 * Read the scratch of a DS28EA00.
 */
short ReadTempScratch(long session_handle, uchar *buf)
{
   ...
   for (i = 0; i < 8; i++)   
   {
      ...
      //dowcrc(buf[i]);
      CRC8 = TMCRC(1, &buf[i], (ushort)CRC8, 0); /* perform CRC on result */
   }
   
   /* check crc */
   CRCByte = (unsigned char) TMTouchByte(session_handle, (short) 0xFF);
   if (TMCRC(1, &CRCByte, (ushort)CRC8, 0) != 0)
      return 0;
